...
from preprocessing import PADDING_CONST, generate_body, generate_title, get_dataset_2t, get_dataset_3t
...
x_real, x_fake = generate_title(4000) # get the first 4000 real and fake articles from this dataset
...
_X, _Y = get_dataset_2t() # extract all the articles from the 2nd dataset
X = np.append(X, _X, axis=0)
Y = np.append(Y, _Y, axis=0)

_X, _Y = get_dataset_3t() # extract all the articles from the 3rd dataset
X = np.append(X, _X, axis=0)
Y = np.append(Y, _Y, axis=0)
...
model.fit(X_train, Y_train, batch_size=64, epochs=20, validation_data=(X_test,Y_test), use_multiprocessing=True, callbacks=[tensorboard, checkpoint])
